...
const { update, list, read } = require('./data');
// dependencies
...
worker.gatherAllChecks = () => {
    // get all the checks
    list('check', (err1, checks) => {
        if (!err1 && checks && checks.length > 0) {
            checks.forEach((check) => {
                // read the checkData
                read('check', check, (err2, originalCheckData) => {
...
worker.performCheck = (originalCheckData) => {
    ...
    console.log('timeout : ', requestDetails.timeout);
...
worker.processCheckOutcome = (originalCheckData, checkOutCome) => {
    ...
    update('check', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            if (alertWanted) {
